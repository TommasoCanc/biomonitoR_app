fluidRow(
  column(
    ...
    box(
      ...
      collapsed = TRUE,
      HTML(
        "<h4> Taxa richness is the measure of the number of taxa in a given sample.
      ...
      checkboxInput("richIndex", label = "Run", value = FALSE),
      # <- Taxa richness
      selectInput(
        "richness.tax_lev",
        "Taxa level",
    ...
    box(
      ...
      collapsed = TRUE,
      HTML(
        "<h4>A diversity index is a helpful measure to quantify how many differences exist between two or more datasets.
      ...
      HTML(
        "<h5> <b>Please note</b>: If richness is below 3, the indices can not be calculated. </h5>"
      ),
      selectInput(
        "div.tax_lev",
        "Taxa level",
    ...
    box(
      ...
      collapsed = TRUE,
      HTML(
        "<h4> Principal Component Analysis (PCA) is a statistical technique
